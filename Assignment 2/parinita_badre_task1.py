...
if __name__ == '__main__':
    ...
    input = sc.textFile(inputFile, minPartitions=None)
    ...
    if case == 1:
        totalBaskets = inputchunk.map(lambda x: (int(x[0]), int(x[1]))).groupByKey().mapValues(set).map(lambda x: x[1])

    else:
        totalBaskets = inputchunk.map(lambda x: (int(x[1]), int(x[0]))).groupByKey().mapValues(set).map(lambda x: x[1])
